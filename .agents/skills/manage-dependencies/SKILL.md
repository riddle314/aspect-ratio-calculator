...
* **Kotlin & KSP**: Ensure the KSP plugin version aligns with Kotlin's major/minor version (e.g. KSP `2.3.x` for Kotlin `2.3.x`). Starting with KSP 2.3+, KSP patch releases follow independent semantic versioning and do not need to match Kotlin's exact patch version.
