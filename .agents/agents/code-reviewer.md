...
3. **Dependency Guidelines**: Reference the [manage-dependencies Skill](../skills/manage-dependencies/SKILL.md) when reviewing changes that introduce, modify, or upgrade libraries and `gradle/libs.versions.toml`.
4. **Build & Test Validation**: If necessary, trigger build or test execution via Gradle using the standard Gradle commands (e.g., `./gradlew test`) to verify test runs.
...
5. **Verify Dependencies & Version Catalogs**: Ensure all dependencies adhere to the [manage-dependencies Skill](../skills/manage-dependencies/SKILL.md).
6. **Verify Tests**: Ensure appropriate unit tests have been added or updated in the corresponding `test/` directory.
7. **Generate Review Report**: Output a structured code review report.
...
### 5. Dependency & Version Catalog Hygiene
* **Centralized Declarations**: Ensure **all dependencies and plugins** are declared in `gradle/libs.versions.toml` (`libs.*`). Disallow hardcoded artifact coordinate strings (e.g., `"com.example:foo:1.0"`) in any `build.gradle.kts` file.
* **Scoping**: Verify appropriate configurations are used (`implementation` for internal dependencies, `api` only when deliberately exposed, `testImplementation` for JVM unit tests).
* **Upgrade Safety**: If library versions are bumped, verify risk tiers and ensure compatibility matrices (Kotlin, KSP, Compose, AGP) remain intact as defined in [manage-dependencies](../skills/manage-dependencies/SKILL.md).
